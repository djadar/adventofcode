...
struct Server {
    ...
    //attached_devices: Vec<Vec<String>>,
    number_of_devices: usize,
    you_device: usize, //its position
...
pub fn read_document(file_path: &str) -> io::Result<Server> {
    ...
    for line in reader.lines() {
        ...
        if block.len() == 2 {
            ...
            if device == "you" {
                server.you_device = server.number_of_devices;
            }
            //server.device_list.push(device);

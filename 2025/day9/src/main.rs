...
impl Tile {
    ...
    fn add_green(&mut self) {
        println!("Step 2: Adding green points in parallel...");
        let mut added = 0usize;

        // Snapshot sets for fast membership checks
        let coord_set: HashSet<(usize, usize)> = self.coordinates.iter().copied().collect();
        let reds = self.red_coordinates.clone();

        // Parallel pair traversal
        let mut new_points: Vec<(usize, usize)> = (0..reds.len())
            .into_par_iter()
            .flat_map(|i| {
                let (mut x1, mut y1) = reds[i];
                let mut local = Vec::new();

                for j in (i + 1)..reds.len() {
                    let (mut x2, mut y2) = reds[j];
                    let height = (x2 as isize - x1 as isize).abs() as usize;
                    let width = (y2 as isize - y1 as isize).abs() as usize;
        ...
        // Fill downward from red points (parallelized by column)
        let mut vertical_points: Vec<(usize, usize)> = (0..self.number_of_columns)
            .into_par_iter()
            .flat_map(|j| {
                let mut local = Vec::new();
                let mut found_red = false;
                for i in 0..self.number_of_lines {
                    ...
                    if found_red && !coord_set.contains(&p) {
                        local.push(p);
                    }
                }
                local
        ...
        // Merge results
        new_points.append(&mut vertical_points);
        ...
        println!("Step 2 complete: {} green points added.", added);
        //self.print(false);
    }

    fn find_largest_rectangle_withinbox1(&mut self) -> usize{
    ...
    fn find_largest_rectangle_withinbox(&mut self) -> usize {
        println!("Step 3: Searching largest rectangle in parallel...");
        ...
        // Parallelize over indices of red coordinates
        ...
        if let Some(first) = self.rectangles.first() {
            println!("{:?} : max area is {}", first, max_area);
        }
        println!("Rectangles found: {}", self.rectangles.len());
